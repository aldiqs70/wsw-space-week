...
**الجمعية الفلكية الأردنية** · الإصدار 0.4 (واجهة منشورة، الحسابات مغلقة) · أيلول 2026
...
### حالة النشر الحالية (0.4)
الموقع منشور كموقع **تعريفي**: `accountsEnabled: false` في `config.js`.
- كل المحتوى العام ظاهر، وصفحتا الدخول والتسجيل تعرضان "التسجيل يفتح قريبًا"، وكل مسارات الطالب والإدارة تعيد إلى الرئيسية، وأي جلسة قديمة محفوظة في المتصفح تُتجاهل.
- أُزيل شريط "نسخة تجريبية" وجدول الحسابات التجريبية. المستخدمون التجريبيون ما زالوا في `mock-data.js` لأغراض التطوير فقط، ولا يعملون ما دام المفتاح `false`.
- **التفعيل:** بعد ربط الـBackend غيّر المفتاح إلى `true`. قبل ذلك لا تفعّله على الموقع المنشور، لأن البيانات حينها تُحفظ في متصفح كل زائر فقط.

### ما الجديد في 0.3
- **كل المحتوى الحالي عام** لكل الزوار بدون تسجيل دخول. أُضيفت صفحة عامة «المراجع والمختبرات» وصفحة «مراجع الطلبة» (للطلبة المسجّلين فقط). الفصل بينهما بحقل `visibility` في جدول `pages` (القسم 6).
- **مكوّن جديد `lab`** (مختبر تفاعلي) يفتح صفحة HTML مستقلة بملء الشاشة. المختبرات ملفات ثابتة في مجلد `labs/` (القسم 10.3).
- **الروابط الداخلية:** حقول `url` في المكوّنات تقبل الآن مسارًا داخل الموقع (`labs/…`) أو صفحة في المنصة (`#/p/…`) إضافة إلى `https://`.
- **قائمة الطالب ديناميكية:** تعرض كل الصفحات المنشورة التي يحق له رؤيتها، على المسار `#/student/p/:slug`.
...
assets/img/labs/            صور مصغّرة لبطاقات المختبرات
labs/                       المختبرات التفاعلية (تُرفع مرة واحدة، مستقلة عن تحديثات الموقع)
  mars-hohmann/index.html   + de421.bsp في المجلد نفسه (16 ميغابايت)
  rockets/index.html
  roman-l2/index.html
```
...
- `public`: يراها كل الزوار. `students`: طالب بحالة `active` فقط (وأي محرر/مشرف). غيرهم → `401`.
- صفحتا `home` و`learn` هما `system=true`: لا تُحذفان. `home` عامة ومنشورة دائمًا؛ رؤية `learn` قابلة للتغيير.
- القيمة الافتراضية لصفحة جديدة: `public`.
...
### 10.3 المختبرات التفاعلية (مجلد labs/)
- كل مختبر صفحة HTML مستقلة في مجلد خاص به، يُرفع كما هو بجانب `index.html` و`assets/` في جذر الموقع. لا يحتاج أي شيء من الـBackend، ولا يمر عبر الـAPI.
- **مختبر المريخ يتطلب `de421.bsp` في المجلد نفسه**، ويحمّله تلقائيًا عبر `fetch('./de421.bsp')`. هذا لا يعمل إلا عبر خادم HTTP(S)؛ فتح الملف مباشرة من الجهاز (`file://`) يُظهر "Kernel not loaded". الملف ثنائي (DAF/SPK) من JPL؛ لا تعدّله ولا تضغطه.
- إذا استُخدم لاحقًا File Storage أو CDN للمختبرات: أبقِ `index.html` و`de421.bsp` في المسار نفسه، واسمح بـ CORS إن اختلف النطاق.
- المختبرات تحفظ تقدّم الطالب في متصفحه فقط (localStorage داخل كل مختبر)، وليست مرتبطة بحسابه في المنصة. ربطها يحتاج تعديل ملفات المختبرات نفسها.
- ملاحظة لمطوّر مختبر المريخ: الملف يرمي خطأ `v61RefreshNavTree is not defined` عند التحميل (غير مؤثر على العمل).
...
- الأنواع الحالية: `text, image, video, link, file, simulation, card, gallery, notice, steps, mission, lab, assignment`.
- حقول `url`: `https://…`، أو `#/…`، أو مسار داخل الموقع بأحرف `[a-z0-9._/-]` بدون `..`. أي بروتوكول آخر (`javascript:`، `data:`) يُرفض. طبّق القاعدة نفسها في الـBackend. نوع غير معروف ← `validation`.
