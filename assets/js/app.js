...
(function () {
  ...
  function appShell(kind) {
    const u = state.user;
    const nav = kind === 'admin' ? adminNav(u) : [
      ['#/student', 'student.nav.home', 'الرئيسية'],
      ['#/student/learn', 'student.nav.learn', 'المحتوى التعليمي'],
      ...
      <div class="app app--${kind}">
        <aside class="sidebar">
          ...
          <nav class="sidenav" aria-label="قائمة ${kind === 'admin' ? 'الإدارة' : 'الطالب'}">
            ${nav.map(([h, k, l]) => `<a href="${h}" data-nav="${h}">${T(k, l)}</a>`).join('')}
          </nav>
  ...
  async function render() {
    ...
    const layoutKey = layout + ':' + (u ? u.id + u.role : 'anon');
    if (currentLayout !== layoutKey) {
      root.innerHTML = (layout === 'student' || layout === 'admin') ? appShell(layout) : await publicShell();
