...
(function () {
  ...
  const api = {
    ...
    async register(data) {
      await wait();
      const db = load();
      if (!db.settings.registrationOpen) throw new ApiError('closed', 'التسجيل مغلق حاليًا.');
    ...
    async createPage(data) {
      ...
      const p = { id: uid('pg'), slug, title: data.title.trim(), visibility: data.visibility || 'students', status: data.status || 'draft', system: false };
      db.pages.push(p);
