...
(function () {
    ...
    main.innerHTML = `
      <section class="hero">
        ...
        <div class="hero__main">
          <div class="hero__content">
            ...
            <div class="hero__actions">
              ${!WSW.config.accountsEnabled ? `<p class="hero__soon">${T('hero.soon', 'يفتح التسجيل في المنصة ' + WSW.config.registrationOpensText)}</p><a class="btn btn--primary btn--large" href="#/p/resources">${T('hero.labs', 'جرّب المختبرات التفاعلية')}</a><a class="btn btn--onDark btn--large" href="#/p/learn">${T('hero.learn', 'المحتوى التعليمي')}</a>`
          ...
          <div class="hero__facts">
            <dl>
              ...
              ${!WSW.config.accountsEnabled
                ? `<div><dt>${T('hero.factOpen.label', 'فتح التسجيل')}</dt><dd>${T('hero.factOpen.value', WSW.config.registrationOpensText)}</dd></div>
              <div><dt>${T('hero.fact3.label', 'إعلان النتائج')}</dt><dd>${T('hero.fact3.value', 'نيسان')}</dd></div>`
        ...
        <figure class="hero__photo">
          <picture>
            <source type="image/webp" srcset="assets/img/hero-launchpad-720.webp 720w, assets/img/hero-launchpad-1100.webp 1100w" sizes="(max-width: 860px) 100vw, 45vw">
            <img src="assets/img/hero-launchpad-1100.jpg" srcset="assets/img/hero-launchpad-720.jpg 720w, assets/img/hero-launchpad-1100.jpg 1100w" sizes="(max-width: 860px) 100vw, 45vw"
              alt="صاروخ SLS على منصة الإطلاق في مركز كينيدي، والقمر بدر فوقه" width="1100" height="1502" fetchpriority="high">
          </picture>
          <figcaption class="hero__credit">${T('hero.photoCredit', 'الصورة: NASA/Cory Huston')}</figcaption>
        </figure>
      </section>
  ...
  /* ---------- الحسابات مغلقة مؤقتًا ---------- */
  const accountsSoon = main => {
    main.innerHTML = `
      <div class="auth"><div class="auth__card status-card status-card--pending">
        <h1>${T('soon.title', 'التسجيل يفتح قريبًا')}</h1>
        <p>${T('soon.text', 'يُفتح باب التسجيل في المنصة للطلبة المرشّحين من مدارسهم ' + WSW.config.registrationOpensText + '. تابعوا إعلانات الجمعية.')}</p>
        <p class="muted">${T('soon.meanwhile', 'حتى ذلك الحين، كل المحتوى التعليمي والمختبرات التفاعلية متاحة للجميع.')}</p>
        <div class="form-actions"><a class="btn btn--primary" href="#/p/resources">${T('hero.labs', 'جرّب المختبرات التفاعلية')}</a><a class="btn btn--ghost" href="#/">${T('nav.home', 'الرئيسية')}</a></div>
      </div></div>`;
  };

  /* ---------- الدخول ---------- */
  WSW.route('/login', async main => {
    if (!WSW.config.accountsEnabled) return accountsSoon(main);
