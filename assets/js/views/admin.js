  ...
  WSW.route('/admin/content', async main => {
    ...
    main.innerHTML = `
      ...
      <div class="table-wrap"><table class="table">
        ...
        <tbody>${pages.map(p => `<tr class="is-link" data-href="#/admin/content/${esc(p.id)}">
          ...
          <td dir="ltr" class="muted">${p.slug === 'home' ? '#/' : '#/p/' + esc(p.slug)}</td>
          <td><button class="btn btn--small btn--ghost" data-inline="${esc(p.slug)}">تحرير على الصفحة</button></td>
          <td>${p.visibility === 'public' ? 'كل الزوار' : '🔒 الطلبة المسجّلون'}</td><td>${publishPill(p.status)}</td></tr>`).join('')}</tbody>
      </table></div>`;
    ...
    main.querySelector('[data-new-page]').addEventListener('click', () => modal({
      ...
      body: `<form novalidate>
        ...
        <div class="field"><label for="visibility">من يراها</label><select id="visibility" name="visibility">${options([['public', 'كل الزوار (بدون تسجيل دخول)'], ['students', 'الطلبة المسجّلون فقط']], 'public')}</select></div>
  ...
  function pageSettings(page, done) {
    modal({
      ...
      body: `<form novalidate>
        ...
        <div class="grid-2">
          <div class="field"><label for="visibility">من يراها</label><select id="visibility" name="visibility" ${page.slug === 'home' ? 'disabled' : ''}>${options([['public', 'كل الزوار (بدون تسجيل دخول)'], ['students', 'الطلبة المسجّلون فقط']], page.visibility)}</select></div>
          <div class="field"><label for="status">الحالة</label><select id="status" name="status" ${page.slug === 'home' ? 'disabled' : ''}>${options([['published', 'منشورة'], ['draft', 'مسودة']], page.status)}</select></div>
        </div>
        ${page.slug === 'home' ? '<p class="hint">الصفحة الرئيسية تظهر دائمًا لكل الزوار؛ يمكن تعديل عنوانها ومحتواها فقط.</p>' : page.system ? '<p class="hint">صفحة أساسية في النظام: لا يمكن حذفها، لكن يمكن تغيير من يراها.</p>' : ''}
        <div class="form-actions">
      ...
      onMount(el, close) {
        ...
        f.addEventListener('submit', async e => {
          e.preventDefault();
          const d = formData(f);
          if (page.slug === 'home') { delete d.visibility; delete d.status; }
