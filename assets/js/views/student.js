...
(function () {
  ...
  /* ---------- المحتوى التعليمي ---------- */
  WSW.route('/student/learn', async main => {
    const { page, sections } = await api.getPage('learn');
    main.innerHTML = `
      <header class="dash-head"><h1>${esc(page.title)}</h1></header>
      ${sections.length ? `
        <nav class="toc" aria-label="أقسام الصفحة">${sections.map(s => `<a href="#/student/learn" data-jump="s-${esc(s.id)}">${esc(s.title)}</a>`).join('')}</nav>
        <div class="cms cms--app">${WSW.renderSections(sections)}</div>`
        : empty('لا يوجد محتوى بعد', 'سيضيف فريق الجمعية الدروس والأنشطة قريبًا.')}`;
