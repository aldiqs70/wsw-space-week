...
(function () {
  const { esc, safeUrl, paragraphs, fmtDate } = WSW.ui;
  ...
  const types = {
    ...
    file: {
      ...
      render: d => `<a class="c-link c-link--file" href="${esc(safeUrl(d.url))}" target="_blank" rel="noopener" download>
          <span class="c-link__title">${esc(d.title)}</span>
          ${d.description ? `<span class="c-link__desc">${esc(d.description)}</span>` : ''}
    ...
    simulation: {
      ...
      render: d => hostAllowed(d.url)
        ...
        : `<a class="c-link" href="${esc(safeUrl(d.url))}" target="_blank" rel="noopener"><span class="c-link__title">${esc(d.title)}</span><span class="c-link__host">فتح المحاكاة في نافذة جديدة</span></a>`
    },
    ...
    card: {
      ...
      render: d => {
        ...
        return safeUrl(d.url)
          ? `<a class="c-card" href="${esc(safeUrl(d.url))}" target="_blank" rel="noopener">${inner}</a>`
    ...
    mission: {
      ...
      render: d => `<article class="c-mission">
          ...
          <footer>${d.date ? `<span>أُطلقت ${esc(fmtDate(d.date))}</span>` : '<span></span>'}${safeUrl(d.url) ? `<a href="${esc(safeUrl(d.url))}" target="_blank" rel="noopener">المزيد عن المهمة</a>` : ''}</footer>
        </article>`
    },
    ...
    assignment: {
      ...
      render: d => `<aside class="c-assignment">
          ...
          <a class="btn btn--primary" href="#/student/projects/new">ابدأ مشروعًا</a>
        </aside>`
    }
  };
  ...
  function validate(type, data) {
    ...
    def.fields.forEach(f => {
      ...
      if (f.type === 'url' && v && !/^https?:\/\//i.test(v)) errors[f.key] = 'الرابط يجب أن يبدأ بـ https://';
    });
    return errors;
  }
  ...
  function formFields(type, data = {}) {
    ...
    return def.fields.map(f => {
      ...
      if (f.type === 'textarea' || f.type === 'lines')
        ...
        input = `<input id="${id}" name="${f.key}" type="${f.type === 'url' ? 'url' : f.type}" value="${esc(v)}" ${f.type === 'url' ? 'dir="ltr" placeholder="https://"' : ''}>`;
